...
class QuentContext:
    """
    Serializable Quent context shared by all ranks.

    Notes
    -----
    ``output_root`` must be the same writable shared-filesystem path on the
    driver and every worker. Direct filesystem export is a temporary workaround
    until Quent provides supported Python bindings for its Collector.
    """
    ...
    def run_root(self) -> Path:
        """Return this engine's shared filesystem output directory."""
