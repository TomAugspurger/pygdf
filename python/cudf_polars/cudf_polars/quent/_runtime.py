...
class QuentSession:
    """Own one filesystem-backed generated context and its active FSM handles."""

    def __init__(self, output_root: str | PathLike[str]) -> None:
        if _quent is None:
            raise ImportError(
        ...
        self._context = _quent.Context(_quent.ExporterOptions.ndjson(output_root))
        self._closed = False
    ...
    def close(self) -> None:
        """Close active handles and flush generated events to the filesystem."""
        if self._closed:
            return
        self._engines.clear()
        self._workers.clear()
