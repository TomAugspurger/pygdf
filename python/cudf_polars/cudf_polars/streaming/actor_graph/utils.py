    ...
    *channels: Channel[Any],
    ...
    if ir_context is not None:
        ...
        if quent_ir_execution_context is not None:
            # TODO: Validate that all this is correct.
            token = uuid.uuid4()

            quent_task = cudf_polars.quent.Task(
                instance_name=f"Actor-{type(trace_ir).__name__}-{quent_ir_execution_context.quent_operator.id.hex[:8]}-{token.hex[:8]}",
    ...
    with cudf_polars.dsl.tracing.bound_contextvars(**contextvars):
        ...
        try:
            ...
            if quent_ir_execution_context is not None:
                ...
                if tracer.chunk_count is not None:
                ...
                if tracer.duplicated is not None:
                    custom_attributes.append(
                        cudf_polars.quent._types.StatisticsAttribute(
                            key="duplicated",
                            value_type="U64",
                ...
                quent_ir_execution_context.logger.emit(
                    quent_ir_execution_context.quent_operator.statistics(
                        statistics=cudf_polars.quent.Statistics(
                            output_rows=tracer.row_count or 0,
                            input_bytes=0,
                            output_bytes=0,
