...
if TYPE_CHECKING:
    ...
    class ChannelMetricsSnapshot(Protocol):
        """Typed boundary for RapidsMPF's unmarked Cython package."""

        @property
        def send_bytes(self) -> Mapping[object, int]:
            """Return bytes sent, grouped by memory type."""
            ...

        @property
        def recv_bytes(self) -> Mapping[object, int]:
            """Return bytes received, grouped by memory type."""
            ...
...
def record_channel_metrics(
    ...
    *,
    ...
    tracer.input_bytes += sum(
        sum(cast("ChannelMetricsSnapshot", channel.metrics()).recv_bytes.values())
        for channel in chs_in
    )
    tracer.output_bytes += sum(
        sum(cast("ChannelMetricsSnapshot", channel.metrics()).send_bytes.values())
        for channel in chs_out
    )
