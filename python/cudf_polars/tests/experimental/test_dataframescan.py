...
def test_parallel_dataframescan(df, max_rows_per_partition, engine):
    ...
    stats = collect_statistics(qir, ConfigOptions.from_polars_engine(_engine))
    ir, info = lower_ir_graph(qir, ConfigOptions.from_polars_engine(_engine), stats)
    count = info[ir].count
...
def test_join_in_memory_lazy_stable_id_pickle():
    ...
    stats = collect_statistics(qir, config)
    ir, _ = lower_ir_graph(qir, config, stats)
...
def test_dataframescan_pickle(df):
    ...
    qir = Translator(df._ldf.visit(), _engine).translate_ir()
    config = ConfigOptions.from_polars_engine(_engine)
    stats = collect_statistics(qir, config)
    ir, _ = lower_ir_graph(qir, config, stats)
