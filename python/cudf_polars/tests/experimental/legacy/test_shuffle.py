...
def test_hash_shuffle(df: pl.LazyFrame, engine: pl.GPUEngine) -> None:
    ...
    stats = collect_statistics(qir2, options)
    partition_info = lower_ir_graph(qir2, options, stats)[1]
    ...
    stats = collect_statistics(qir3, options)
    partition_info = lower_ir_graph(qir3, options, stats)[1]
