...
1. common tests, which are run by each engine. These build some polars query and
   execute it using the provided engine. Our test runner will run the test
   multiple times, once per engine.
2. engine-specific tests, which execercise some very specific aspect of an
   engine. The tests might build a polars query, or unit test some other
   functionality from `cudf-polars`. Either way, they'll also construct and
   use an engine with very specific parameters.
